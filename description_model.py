...
def describe_image(user_prompt, sent_image):
    image = Image.open(sent_image)
    enc_image = model.encode_image(image)
    return model.answer_question(enc_image, user_prompt, tokenizer)
    #return model.answer_question(enc_image,f"{prompt_prefix_3} {user_prompt}", tokenizer)
